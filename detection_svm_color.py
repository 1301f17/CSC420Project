...
def train(xs1, ys1, fs1, xs2, ys2, fs2):
	xs1 = xs1.reshape(-1, H * W * 3)
	pca = PCA(n_components=100)
	pca.fit(xs1)
	xs1 = pca.transform(xs1)
	with open('model1_pca_color.pkl', 'wb') as f:
		pickle.dump(pca, f)
	...
	clf.fit(xs1, ys1)
	with open('model1_svm_color.pkl', 'wb') as f:
		pickle.dump(clf, f)

	xs2 = xs2.reshape(-1, H * W * 3)
	pca = PCA(n_components=100)
	pca.fit(xs2)
	xs2 = pca.transform(xs2)
	with open('model2_pca_color.pkl', 'wb') as f:
		pickle.dump(pca, f)
	...
	clf = CalibratedClassifierCV(clf)
	clf.fit(xs2, ys2)
	with open('model2_svm_color.pkl', 'wb') as f:
		pickle.dump(clf, f)
...
def cross_validation():
	...
	with open('model1_svm_color.pkl', 'rb') as f:
		clf = pickle.load(f)
	with open('model1_pca_color.pkl', 'rb') as f:
		pca = pickle.load(f)		

	fs2 = np.load('fs2.npy')
	for sub_folder in fs2[2:]:
		print(sub_folder)
	...
	with open('model2_svm_color.pkl', 'rb') as f:
		clf = pickle.load(f)
	with open('model2_pca_color.pkl', 'rb') as f:
		pca = pickle.load(f)
